...
class Order(models.Model):
  ...
  def get_or_set_shipping_address(self):
    ...
    if shipping_address:
      self.update_shipping_address(shipping_address) #Ocupa el metodo "update_shipping_address" para guardar la dirección.
    
    return shipping_address
  
  def update_shipping_address(self, shipping_address):  # Recibe como argumento una dirección.
    self.shipping_address = shipping_address  #Guarda la dirección en el atributo shipping_address de 'Order'
    self.save()

  def cancel(self):
    self.status = OrderStatus.CANCELED
    self.save() # Actualiza el registro
